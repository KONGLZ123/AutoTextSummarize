...
i = 0
chars = {}
infile = open('E://Python_Code//bytecup//data//bytecup.corpus.train.0.txt', 'r', encoding='utf-8', errors='ignore')
for line in infile.readlines():
    lineParsed = json.loads(line.strip('\n'))

    for w in lineParsed['content'].strip().split(' '):  # 纯文本，不用分词
        chars[w] = chars.get(w, 0) + 1
    for w in lineParsed['title'].strip().split(' '):  # 纯文本，不用分词
        chars[w] = chars.get(w, 0) + 1
...
def data_generator():
    ...
    for line in infile.readlines():
        lineParsed = json.loads(line.strip('\n'))
        X.append(str2id(lineParsed['title'].strip()))
        Y.append(str2id(lineParsed['content'].strip()))
