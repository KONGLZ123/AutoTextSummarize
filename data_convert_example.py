...
tf.app.flags.DEFINE_string('in_file', './bytecup2018/valid.txt', 'path to file')
tf.app.flags.DEFINE_string('out_file', './bytecup2018/validition-0', 'path to file')
...
def _text_to_binary():
  ...
  for inp in inputs:
    ...
    for feature in inp.strip().split('\t'):
      (k, v) = feature.split('=')
      tf_example.features.feature[k.encode()].bytes_list.value.extend([v.encode()])
